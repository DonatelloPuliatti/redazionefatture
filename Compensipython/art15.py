...
def art15():
    ...
    danni = float(request.form.get('danni'))
    risultato = 0
    listarisultati = [0, 0, 0, 0, 0, 0, 0]
    primoscaglione = [0, 5164.57, 6.5686, 13.1531]
    secondoscaglione = [5164.58, 10329.14, 4.6896, 9.3951]
    terzoscaglione = [10329.15, 25822.84, 3.7580, 7.5160]
    quartoscaglione = [25822.85, 51645.69, 2.8106, 5.6370]
    quintoscaglione = [51645.70, 103291.38, 1.8790, 3.7580]
    sestoscaglione = [103291.39, 258228.45, 0.9316, 1.8790]
    settimoscaglione = [258228.46, 516456.90, 0.2353, 0.4705]
    ...
    for idx, i in enumerate(listascaglioni):
        if importo > i[0]:
            ...
            risultatoperscaglione = percentualedaapplicare / 100 * sommariferimento/2/danni
            listarisultati[idx] = risultatoperscaglione
    risultato = sum(listarisultati)
    risultatoformattato = "{:.2f}".format(risultato)
    if risultato < 96.58:
        risultatoformattato = "145.12, in quanto ai sensi dell'art. 15, c. 2, il compenso non può essere inferiore ad € 96.58"
    if importo > settimoscaglione[1]:
        risultatoformattato = f"{risultatoformattato}, ovvero il compenso da corrispondere quando l'importo della perizia/consulenza è pari ad € 516.456.90. Si tenga presente che nel D.M. non è previsto uno scaglione finale con valore illimitato"
...
"""
    ...
    ipotesiselezionata = "La fattispecie selezionata è la 'perizia o la consulenza tecnica in materia di valutazione, riparazione e trasformazione di aerei, navi e imbarcazioni e in quella di salvataggio e recuperi', disciplinata dall'art. 15 D.M. 30 maggio 2002"
    if compenso == 0:
    ...
    correttivo = ""
    if danni == 2:
        correttivo = "Si è fatta applicazione della riduzione di cui all'art. 15, c. 1, secondo periodo, D.M. 30 maggio 2002, in quanto la perizia/consulenza consiste in una valutazione dei danni."
